...
MCSignal* o2::aod::dqmcsignals::GetMCSignal(const char* name)
{
  ...
  // analysis-muon-selection -> "cfgMuonMCSignals": "PowhegDYMuon1"
  if (!nameStr.compare("PowhegDYMuon1")) {
    ...
    signal = new MCSignal(name, "POWHEG Muon singles", {prong}, {-1});// define a signal with one prong
    return signal;
  }
  ...
  if (!nameStr.compare("PowhegDYMuon2")) {
    MCProng prong(2, {13,2212}, {true,true}, {false,false}, {0,0}, {0,0}, {false,false}); // selecting dimuons
    prong.SetSourceBit(0, MCProng::kIsPowhegDYMuon);  // set source to be Muon from POWHEG
    signal = new MCSignal(name, "POWHEG Muon pair", {prong,prong}, {-1,-1});// define a signal with two prong
    return signal;
  }
